...
exports.getUserPerformance = exports.saveInterview = void 0;
...
const saveInterview = async (req, res) => {
    try {
        ...
        const overallScore = Number(body.evaluation.overallScore);
        ...
        const grade = typeof body.evaluation.grade === "string"
            ? body.evaluation.grade
            : undefined;
        const recommendation = typeof body.evaluation.recommendation ===
            "string"
        ...
        if (!user) {
            ...
            if (body.resumeUrl?.trim())
                user.resumeUrl = body.resumeUrl.trim();
            await user.save();
        }
        ...
        const updatedUser = await User_1.User.findByIdAndUpdate(user._id, { $inc: { totalInterviews: 1 } }, { new: true });
        res.json({
            success: true,
...
const getUserPerformance = async (req, res) => {
    try {
        ...
        const sessions = await InterviewSession_1.InterviewSession.find({ userId: user._id })
            .sort({ completedAt: -1 })
            .limit(10)
            .lean();
        ...
        const mcqTrend = lastFive.map((s) => {
            ...
            const pct = total > 0 ? Math.round(((snap?.mcqScore || 0) / total) * 100) : 0;
            return {
                label: new Date(s.completedAt).toLocaleDateString(undefined, {
                    month: "short",
                    day: "numeric",
        ...
        const accumulateRadar = () => {
            if (!lastFive.length)
                return null;
            const keys = ["mcq", "coding", "communication", "behavioral"];
