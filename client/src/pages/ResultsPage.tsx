...
const FALLBACK_EVALUATION: EvaluationResult = {
  ...
  grade: "B+",
  summary:
    "The candidate demonstrated solid understanding of core CS concepts. Code submissions showed working implementations with some room for optimization.",
  strongPoints: [
    {
      point: "Good MCQ performance",
      evidence: "Answered most fundamental CS questions correctly.",
    },
    {
      point: "Clear communication",
      evidence: "Articulated technical concepts well in the chat.",
    },
    {
      point: "Working code",
      evidence: "Submitted functioning solutions for DSA problems.",
    },
  ],
  weakPoints: [
    {
      point: "Time complexity analysis",
      suggestion:
        "Practice explaining Big-O notation for your solutions out loud.",
    },
    {
      point: "Edge case handling",
      suggestion:
        "Always consider null inputs, empty arrays, and boundary conditions.",
    },
    {
      point: "Code optimization",
      suggestion: "Review common patterns like two-pointer and sliding window.",
    },
  ],
  codeQuality: {
    score: 70,
    ...
    readability: 80,
    feedback:
      "Code was readable and structured. Consider adding comments and handling edge cases.",
  },
  sectionScores: {
    mcq: 80,
    coding: 65,
  ...
  recommendation: "Hire",
  nextSteps:
    "Focus on dynamic programming patterns and practice articulating trade-offs in system design questions.",
};

// ─── Learning Resources Component ────────────────────────────────────────────
function ResourceCard({ resource }: { resource: Resource }) {
  return (
    <div className="apple-card dark:neon-border p-4 flex flex-col gap-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {resource.youtube && (
          ...
          >
            <Youtube size={12} />
            {resource.youtubeLabel || "Watch Video"}
            <ExternalLink size={10} />
          </a>
        )}
      </div>
...
function LearningResources({
  ...
  weakPoints: string[];
  sessionData: { codeSubmissions?: { question: string }[] };
}) {
  // Gather resources from weak point keywords
  const weakResources: { label: string; resource: Resource }[] = [];
  const seen = new Set<string>();

  for (const point of weakPoints) {
    const res = getResourcesForWeakPoint(point);
    if (res && !seen.has(res.topic)) {
      seen.add(res.topic);
      weakResources.push({ label: point, resource: res });
    }
  }

  // Gather resources from submitted DSA questions
  const dsaResources: Resource[] = [];
  const dsaKeys = Object.keys(QUESTION_RESOURCES).filter(
    (k) =>
      k.includes("dsa") ||
      ["m_dsa_1", "m_dsa_2", "h_dsa_1", "h_dsa_2", "e_dsa_1"].includes(k),
  );
  for (const key of dsaKeys) {
    const res = QUESTION_RESOURCES[key];
    if (res && !seen.has(res.topic)) {
      seen.add(res.topic);
      dsaResources.push(res);
    }
  }

  const hasContent = weakResources.length > 0 || dsaResources.length > 0;
    ...
    >
      <h2 className="font-bold text-[var(--text)] mb-5 flex items-center gap-2">
        ...
        <span className="text-xs font-normal text-[var(--text-muted)] ml-1">
          — curated links to help you level up
        </span>
      </h2>

      {weakResources.length > 0 && (
        <>
          <p className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)] mb-3">
            Based on your weak areas
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-5">
            {weakResources.map(({ resource }, i) => (
      ...
      {dsaResources.length > 0 && (
        <>
          <p className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)] mb-3">
            DSA Topics Covered in This Session
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {dsaResources.map((resource, i) => (
              <ResourceCard key={i} resource={resource} />
            ))}
          </div>
        </>
...
export default function ResultsPage() {
  ...
  const usedFallbackEvaluation = useRef(false);
  const didPersistInterview = useRef(false);

  const sessionData = useMemo(
    () => (location.state as object | null) || getSessionData(),
    [location.state, getSessionData],
  );
  ...
  useEffect(() => {
    const fetchEvaluation = async () => {
      try {
        const result = await evaluateSession(
          sessionData as Parameters<typeof evaluateSession>[0],
        );
        usedFallbackEvaluation.current = false;
        setEvaluation(result);
      } catch {
        usedFallbackEvaluation.current = true;
    ...
    fetchEvaluation();
    // intentionally run once on mount with captured session snapshot
  ...
  useEffect(() => {
    const persist = async () => {
      if (
        !evaluation ||
        usedFallbackEvaluation.current ||
        didPersistInterview.current
      )
        return;
      const sd = sessionData as Record<string, unknown>;
      const email =
        typeof sd.candidateEmail === "string"
          ? sd.candidateEmail.trim().toLowerCase()
          : "";
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return;
      didPersistInterview.current = true;
      try {
        await saveCompletedInterview({
          email,
          name:
            typeof sd.candidateName === "string" ? sd.candidateName : undefined,
          difficulty:
            typeof sd.difficulty === "string" ? sd.difficulty : undefined,
          terminated: !!sd.terminated,
          timeUsed: typeof sd.timeUsed === "number" ? sd.timeUsed : 0,
          evaluation,
          sessionSnapshot: {
            ...sd,
            chatHistory: Array.isArray(sd.chatHistory)
              ? (sd.chatHistory as unknown[]).slice(-40)
              : sd.chatHistory,
          },
          proctoring: {
            tabSwitchCount: Number(sd.tabSwitchCount) || 0,
            faceViolationCount: Number(sd.faceViolationCount) || 0,
            noiseAlertCount: Number(sd.noiseAlertCount) || 0,
          },
        });
      } catch {
        didPersistInterview.current = false;
      }
    };
    void persist();
  ...
  const recStyle =
    RECOMMENDATION_COLORS[evaluation.recommendation] ||
      ...
      <div className="relative z-10 max-w-5xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <div
              className={`text-xl font-black px-5 py-3 rounded-2xl border ${recStyle}`}
            >
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-4 mb-4">
            <MetricBar
              label="Correctness"
              value={evaluation.codeQuality.correctness}
              color="#4ade80"
            />
            <MetricBar
        ...
        {/* ── Learning Resources ── */}
        <LearningResources
          weakPoints={evaluation.weakPoints.map((p) => p.point)}
          sessionData={
            sessionData as { codeSubmissions?: { question: string }[] }
          }
        />
